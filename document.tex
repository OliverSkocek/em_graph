...
\begin{definition}
	...
	\end{equation}
	Sei weiters $v\in V$ , eine Ecke dann nennen wir 
		\begin{equation}
...
\\
Durch Anwendung des de'Rham Komplex Theorems haben wir damit gezeigt, dass sich der Imagin\"arteil im Bild der Divergenz befindet und somit mit $\text{div}\text{div}^{-1}$ erweitert werden kann, was uns zum gesuchten Resultat f\"uhrt.
\begin{equation*}
...
\begin{definition}
	...
	\begin{equation}
		\hat{H}=-\sum_{k\in V}2\langle\partial,\partial\rangle_{\mathfrak{E},\mathbb{C}} + \frac{1}{2}(\langle r^{-1}\text{grad}_r^a(\Phi), r^{-1}\text{grad}_r^a(\Phi)\rangle_{\mathfrak{K},\mathbb{C}}+ m^2\langle\Phi, \Phi\rangle_{\mathfrak{E},\mathbb{C}})
	\end{equation}
	Der Wirtinger-Operator $\partial$ wird definiert durch:
	\begin{equation}
		\partial_j =\frac{1}{2}(\frac{\partial}{\partial a_j} - \frac{\partial}{\partial b_j})
	\end{equation}
...
\noindent
\begin{theorem}
	Sei $G$ ein planarer Graph, $r>0$ und $a$ Kantenfunktionen, dann gibt es eine eindeutige L\"osung der Schr\"odingergleichung des Klein-Gordonfeldes.
\end{theorem}
\begin{proof}
	content...
\end{proof}
