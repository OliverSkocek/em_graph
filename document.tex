...
\begin{definition}
	Sei $G$ ein planarer Graph, $m>0$ eine reele Zahl und $r>0$ sowie $a$ Kantenfunktionen, dann ist der der Hamiltonoperator des Klein-Gordon Feldes gegeben durch:
	\begin{equation}
		\hat{H}=-\sum_{k\in V}2\overline{\partial_k}\partial_k + \frac{1}{2}(\langle r^{-1}\text{grad}^a(\Phi), r^{-1}\text{grad}^a(\Phi)\rangle_{\mathfrak{K}}+ m^2\langle\Phi, \Phi\rangle_{\mathfrak{E}})
	\end{equation}
...
\noindent
\begin{theorem}
	Sei $G$ ein planarer Graph, $r>0$ sowie $a$ Kantenfunktionenn, dann gibt es eine eindeutige L\"osung der Schr\"odingergleichung des Klein-Gordonfeldes.
\end{theorem}
\begin{proof}
	content...
\end{proof}
