...
\begin{equation}
	S(f_1, f_2) = \int_{r}^{s}\sum_{e\in E}W(f_1, f_2, f_1^\prime, f_2^\prime)(e)dt
\end{equation}
Die Funktion unter dem Integral nennt man die \textbf{Lagrangefunktion} der Maxwellgleichung.
$W$ ist dabei definiert durch:
\begin{equation}
	W(q_1, q_2, v_1, v_2) = \frac{1}{2}(\epsilon(v_2 + r\text{grad}(q_1))^2 - \text{div}_D(r^{-1}q_2)^2/\mu) + q_2j
\end{equation}
\end{theorem}
\begin{proof}
...
\end{proof}
\noindent
Im n\"achsten Schritt wird diese Lagrangefunktion \"uber eine Legendre Transformation in die zugeh\"orende Hamiltonfunktion umgewandelt. Hierbei werden wir eine Eichung der Potentiale einf\"uhren um das Skalarpotential zu eliminieren. 
\chapter*{Abschluss}
