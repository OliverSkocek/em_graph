...
\begin{definition}
	Sei $G$ ein planarer Graph, $r>0$, $\epsilon$, $e_0$ Kantenfunktionen, $j\in C^1(\mathbb{R}, \mathfrak{K})$ eine Kurve, $\mu$ und $b_0$ Fl\"achenfunktionen und $q_0$ eine Eckfunktion, dann nennt man eine Kurve $e\in C^1(\mathbb{R}, \mathfrak{K})$, eine Kurve $b\in C^1(\mathbb{R}, \mathfrak{F})$  und eine Kurve $e\in C^1(\mathbb{R}, \mathfrak{E})$, sodass f\"ur alle $t\in\mathbb{R}$ gilt:
	\begin{align}
...
\begin{proposition}
Sei $G$ ein planarer Graph, $r>0$, $\epsilon$,  Kantenfunktionen, $j\in C^1(\mathbb{R}, \mathfrak{K})$, $\mu$ eine Fl\"achenfunktion dann sind $a$ und $\phi$ L\"osungen der Maxwellgleichung in Potentialform (5.3), genau dann wenn die Hamiltonschen Bewegunggleichungen erf\"ullt sind:
\begin{align}
...
\end{definition}
\noindent
Zwischen den Operatoren $\hat{\pi}$ und $\hat{q}$ wird die kanonische Vertauschungsrelation gefordert:
\begin{equation}
	[\hat{q}(e), \hat{\pi}(e')] = i\delta_{e, e'}
\end{equation}
f\"ur jede Kante $e\in E$.\\
\\
Eine m\"ogliche Darstellung dieser Operatoren $\hat{q}(e)$ und $\hat{\pi}(e)$ ist gegeben durch:
\begin{align}
	\hat{\pi}(e):(q\mapsto\psi(q)) \mapsto (q\mapsto i\frac{\partial\psi}{\partial q_e}(q))\\
	\hat{q}(e):(q\mapsto\psi(q)) \mapsto (q\mapsto q_e\psi(q))
\end{align}
\noindent
Die Ableitung $\frac{\partial}{\partial q_e}$ bezieht sich dabei auf die zweite Ableitung nach dem Wert der Kante $e\in E$ und $q_e$ ist der Kantenfunktion $q$ an der Stelle $e\in E$.
\\
\\
Eine kurze Betrachtung der Hamiltonfunktion (6.4) zeigt eine \"Aquivalenz des Problems zu einer $|E|$ Punktma\ss{}en mit Massen $r(e)$ f\"ur jedes $e\in E$ in einem Potential. Die Schr\"odingergleichung l\"asst sich entsprechend schreiben als:
\begin{equation}
	-\sum_{e\in E}\frac{1}{2\epsilon(e)}\frac{\partial^2\psi}{\partial q_e^2} + (\frac{1}{2}\langle \text{div}_D(r^{-1}q), \text{div}_D(r^{-1}q)\rangle_{\mathfrak{K}}/\mu - \langle q, j\rangle_{\mathfrak{K}})\psi = i\frac{\partial\psi}{\partial t}
\end{equation}
\noindent
