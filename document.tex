...
\begin{proposition}
	Sei $G$ ein planarer Graph $m>0$ eine reele Zahl, $r>0$, $a$ Kantenfunktionen, $\Phi_0$ sowie $v_0$ komplexe Eckfunktionen und $\pi$, $\Phi\in C^1(\mathfrak{E}, \mathbb{C})$. Weiters schreiben wir:
	\begin{align}
		&\Phi = \Phi_R + i *\Phi_I\\
		&\pi = \pi_R + i *\pi_I\\
	\end{align}
	Die Hamiltonfunktion der Klein-Gordontheorie definieren wird dann als:\footnote{Die Skalarprodukte beziehen sich auf die Standardskalarprodukte der komplexen Vektorr\"aume $\mathbb{C}^E$ und $\mathbb{C}^K$.}
	\begin{equation}
	H(\Phi, \pi) = \frac{1}{2}(\langle \pi, \pi\rangle_{\mathfrak{E},\mathbb{C}}+\langle r^{-1}\text{grad}^a(\Phi), r^{-1}\text{grad}^a(\Phi)\rangle_{\mathfrak{K},\mathbb{C}}+m^2\langle \Phi,\Phi\rangle_{\mathfrak{E},\mathbb{C}})
	\end{equation}	
...
\begin{definition}
	...
	\begin{equation}
		\langle \phi, \xi\rangle_{\mathfrak{H}} = \int_{\mathfrak{E}^2} \overline{\phi}(x)\xi(x)d\lambda(x)
	\end{equation}
\end{definition}
\noindent
