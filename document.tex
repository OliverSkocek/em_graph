...
\begin{proof}
	Sei $\tilde{e}\in E$
	\begin{align*}
		\text{grad}^{a+\text{grad}(\xi)}(e^{i\xi}\Phi)(\tilde{e}) &= e^{i\xi(p(\tilde{e}))}\Phi(p(\tilde{e})) - e^{i\xi(q(\tilde{e}))}(q(\tilde{e}))e^{i(a+\text{grad}(\xi))(\tilde{e})}\\
		&= e^{i\xi(p(\tilde{e}))}(\Phi(p(\tilde{e})) - \Phi(q(\tilde{e}))e^{ia(\tilde{e})})\\
...
\end{proposition}
\begin{proof}
	Nach Hilfsatz 7.2 verbleibt es zu zeigen, dass 
	\begin{equation*}
...
\end{proposition}
\begin{proof}
Starten wir bei der linken Seite von (7.10)
\begin{align*}
