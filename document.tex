...
\begin{align*}
	L(q_1, q_2, v_1, v_2) =&\frac{1}{2}(\epsilon\langle v_2 + r\text{grad}(q_1), v_2 + r\text{grad}(q_1)\rangle_\mathfrak{K} -\\& \langle\text{div}_D(r^{-1}q_2), \text{div}_D(r^{-1}q_2)\rangle_\mathfrak{K}/\mu) + \langle q_2, j\rangle_\mathfrak{K}
\end{align*}
\end{theorem}
\begin{proof}
...
\begin{equation}
	L(q, v) =\frac{1}{2}(\epsilon\langle v, v\rangle_\mathfrak{K} - \langle\text{div}_D(r^{-1}q), \text{div}_D(r^{-1}q)\rangle_\mathfrak{K}/\mu) + \langle q, j\rangle_\mathfrak{K}
\end{equation}
\noindent
...
\begin{equation}
	H(\pi, q) = \frac{1}{2\epsilon}\langle p, p\rangle_{\mathfrak{K}} + \frac{1}{2}\langle \text{div}_D(r^{-1}q), \text{div}_D(r^{-1}q)\rangle_{\mathfrak{K}}/\mu - \langle q, j\rangle_{\mathfrak{K}}
\end{equation}
...
\end{proof}
\noindent
An diesem Punkt entfernen wir uns von der klassischen Theorie und folgen Heisenberg in die Quantenmechanik. Die Kurven $\pi$ und $q$ aus $C^1(\mathbb{R}, \mathfrak{K})$ sind von nun an Kurven in einem Raum von Operatoren definiert auf einem Hilbertraum.
\begin{definition}
	Sei $G$ ein planarer Graph und $r>0$ eine Kantenfunktion, dann ist der Hilbertraum unserer Theorie gegeben durch
	\begin{equation}
		\mathfrak{H} = L^2(\text{kern}(\text{div}_Dr^{-1})^\perp,\mathbb{C})
	\end{equation}
	Der Raum der komplexen Funktionen auf dem reelen Vektorraum\\ $\text{kern}(\text{div}_Dr^{-1})^\perp\subset \mathfrak{K}(G)$ der Quadrate ein endliches Integral \"uber den gesamten Raum haben.
\end{definition}
\noindent
Zwischen den Operatoren $\pi$ und $q$ wird die kanonische Vertauschungsrelation gefordert:
\begin{equation}
	[\pi(e'), q(e)] = i\delta_{e, e'}
\end{equation}
