...
\begin{theorem}
	Die Maxwellgleichungen sind eindeutig l\"osbar, solange ihre Anfangsbedingung (4.1) erf\"ullt sind.
\end{theorem}
\begin{proof}
\end{proof}
\paragraph{Potentialformulierung der Maxwellgleichungen}
Analog zum kontinuierlichen Fall lassen sich die Maxwellgleichungen \"uber Potentiale ausdr\"ucken. 
\begin{definition}
Sei $G$ ein planarer Graph mit Orientierung dann nennt man eine Schar von Kantenfunktion $a$ ein Vektorpotential und eine Schar von Eckfunktion $\phi$ ein Skalarpotential falls f\"ur alle $t\in\mathbb{R}$
	\begin{align}
...
\begin{definition}
	Seien $\phi$ und $a$ Potentiale wie in Definition 4.4, und $\xi$ eine Schar von Eckfunktionen, dann nennt man die Transformation definiert durch
	\begin{align}
...
\end{definition}
\noindent
Die Potential $a$ und $\phi$ sind n\"amlich nicht eindeutig durch die Maxwellgleichungen bestimmt.
\begin{proposition}
	Seien $\phi$, $a$ und $\xi$ wie in Definition 4.5, dann erzeugen $\phi$ und $a$
	nach (4.7) und (4.8) dieselben Felder $e$ und $b$ wie ihre zu $\xi$ geh\"orende Eichtransformation $\hat{\phi}$ und $\hat{a}$.\\
	\\
	Insbesondere bleibt dadurch auch (4.9) unver\"andert unter Eichtransformation.
\end{proposition}
\begin{proof}
...
Wir wollen nun eine Quantenmechanik der Maxwellgleichungen auf planaren Graphen entwickeln.  Nur haben wir in unserer Konstruktion kein Helmoltz Theorem zur Verf\"ugung. $\text{grad}$ und $\text{rot} = \text{div}_D$ zeigen auf unterschiedliche R\"aume und 
