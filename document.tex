...
\end{definition}
\begin{proposition}
	Sei $G$ ein gerichtetes planares Netz und $f \in \mathfrak{K}(G)$ mit $\mathrm{div}_D(f)=0$. 
	Dann gilt: Jeder geschlossene Pfad in $G$ hat Pfadsumme Null.
\end{proposition}
\begin{proof}
Sei $A \in F(G)$ eine Fläche. Dann entspricht die Pfadsumme des gegen den 
Uhrzeigersinn durchlaufenen Randes von $A$ genau dem Wert 
$\mathrm{div}_D(f)(A)$ nach Definition\\
\\
Es genügt, einfach geschlossene Pfade zu betrachten, da jeder kompliziertere 
geschlossene Pfad eine Zusammensetzung solcher ist. 
Sei also $\gamma$ ein einfach geschlossener Pfad in $G$, der im Uhrzeigersinn durchlaufen wird. 
Dieser Pfad zerlegt $G$ in zwei Teilnetze, deren Schnitt genau $\gamma$ ist. 
Da $G$ planar ist, nennen wir den Teil, der nicht den äußeren Rand des Gesamtnetzes enthält, den \emph{inneren Graph} $H$.  
\\
\\
Wir betrachten nun eine Fläche $A \in F(H)$, die eine gemeinsame Kante 
$e \in E$ mit dem Rand von $H$ besitzt. 
Erweitern wir $\gamma$, indem wir anstelle der Randkante $e$ den ganzen 
Randumlauf von $A$ in den Pfad einfügen (im Uhrzeigersinn), so erhalten wir 
einen neuen Randumlauf $\gamma'$, der ebenfalls das Innere von $H$ enthält. 
Dann gilt die Zerlegungsformel
\[
S_H(\gamma)(f) \;=\; S_H(\gamma')(f) \;+\; \mathrm{div}_D(f)(A).
\]
Da $\mathrm{div}_D(f)=0$ gilt, verschwindet der Beitrag $\mathrm{div}_D(f)(A)$. 
Wir können daher $A$ eliminieren, ohne den Wert der Pfadsumme zu ändern. 
Da $H$ nur endlich viele Flächen enthält, wiederholen wir diesen Vorgang 
endlich viele Male, bis keine Fläche mehr übrig bleibt. 
\\
\\
Am Ende dieser Elimination bleibt $S_H(\gamma)(f)=0$. 
Damit verschwindet die Pfadsumme für jeden einfach geschlossenen Pfad, 
und somit auch für beliebige geschlossene Pfade.
\end{proof}
\noindent
\begin{remark}
