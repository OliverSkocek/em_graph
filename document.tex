...
\end{proof}
\noindent
Analog zur Maxwelltheorie finden wir f\"ur die  Klein-Gordongleichung daher folgende Eichtransformation:
\begin{definition}
	Seien $\phi$ und $a$ wie in Definition 7.2, und $\xi\in C^1(\mathbb{R}, \mathfrak{E})$, dann nennt man die Transformation $T$ definiert durch
\begin{equation}
	\gamma_\xi(\left(
	\begin{array}{c}\phi\\
		a\\
...
\begin{definition}
	...
	\begin{equation}
		\hat{H}=-\sum_{k\in V}2\overline{\partial_k}\partial_k + \frac{1}{2}(\langle r^{-1}\text{grad}(\Phi), r^{-1}\text{grad}(\Phi)\rangle_{\mathfrak{K}}+ m^2\langle\Phi, \Phi\rangle_{\mathfrak{E}})
	\end{equation}
