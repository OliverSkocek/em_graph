...
\begin{proposition}[Poincare Inequality]
	...
	$$\langle u, u\rangle_{\mathfrak{E}}\leq \frac{|V|}{\min_{x\notin\partial}r(x)} \langle u, \Delta_r(u)\rangle_{\mathfrak{E}}$$
\end{proposition}
\begin{proof}
	Sei $x\in\partial^c$ and $(x_i)_{i=1}^n$ ein Pfad von $\partial$ nach $x$, dann gilt f\"ur die Pfadsumme:
	$$u(x)^2 = (\sum_{i=1}^{n-1} \sigma_i *\text{grad}(u)(e_i))^2\leq \sum_{i=1}^{n-1}\text{grad}(u)(e_i)^2$$
	Summieren wir nun \"uber $V$ und sch\"atzen weiter ab:
	$$\langle u, u\rangle_{\mathfrak{E}}\leq |V|\langle \text{grad}(u), \text{grad}(u)\rangle_{\mathfrak{K}}\leq\frac{|V|}{\min_{x\notin\partial}r(x)} \langle \text{grad}(u), r*\text{grad}(u)\rangle_{\mathfrak{K}}=$$
	$$\frac{|V|}{\min_{x\notin\partial}r(x)} \langle u, \Delta_r(u)\rangle_{\mathfrak{E}}$$
\end{proof}
\begin{theorem}
...
\begin{proof}
	...
	\\
	Falls aber $\partial$ nicht leer ist, aber $\partial\neq V$ ist, sollen zwei Alternativen gezeigt werden dies zu beweisen:\\
	\\
	\textbf{1. Alternative:} Die Poincare Ungleichung zeigt dies bereits, da der Kern trivial sein muss.\\
	\\
	\textbf{2. Alternative:} Sei $A$ die Untermatrix die man durch streichen der zu $\partial$ geh\"orenden Zeilen und Spalten erh\"alt und sei $x$ ein Element von $\text{kern}(A)$, daher $A(x)=0$. Wir k\"oennen ein Element $\hat{x}$ von $\mathfrak{E}$ konstruieren, sodass $\hat{x}$ an den Ecken die zu $\partial$ geh\"oren Null gesetzt wird und sonst den Wert annimmt den $x$ an der entsprechenden Ecke hat. Es gilt dann:
