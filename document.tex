...
\begin{corollary}
	...
	\end{equation}
	dann ist die L\"osung der Schr\"dingergleichung $\psi$ entlang jeder Richtung des gerade definierten Raums $A$  fast \"uberall konstant.
	\begin{equation}
...
\\
Es wurde somit gezeigt, dass das freie elektromagnetische Feld seine Energie nur in ganzzahligen Vielfachen der Frequenz aufnehmen und abgaben kann. 
...
\begin{equation*}
	\hat{a}_\omega = \sqrt{\frac{\omega}{2}}(\hat{q} + i \frac{1}{\omega}\hat{\pi})
\end{equation*}
...
\end{equation*}
Wir k\"onnen mit Hilfe dieser Operatoren das Eigenwertproblem des eindimensionalen harmonischen Oszillators l\"osen. Hierzu definiere man zun\"achst den Besetzungszahloperator 
\begin{equation*}
...
\end{align*}
Es kann dann gezeigt werden, dass die Eigenwerte des Besetzungszahloperators $\hat{N}_\omega$ genau die nat\"urlichen Zahlen sind und die zugeh\"origen Eigenr\"aume eindimensional sind.
\\
\\
Der Vernichtungsoperator verringert die Besetzungszahl um eins
\begin{equation*}
	\hat{N}\psi = n \psi \rightarrow \hat{N}\hat{a}\psi = (n-1)\hat{a}\psi
\end{equation*} 
Insbesondere gilt $\hat{a}\psi_1=0$, wobei $\psi_1$ der Eigenvektor zum Eigenwert eins ist.\\
\\
Umgekehrt erhöht der Erzeugungsoperator die Besetzungszahl um eins.
\begin{equation*}
...
\end{equation*}
Wir interpretieren nun den Besetzungszahloperator $\hat{N}_\omega$ als die Anzahl der Teilchen oder Photonen der Mode $\omega$ des elektromagnetischen Feldes. Erzeugungs- und Vernichtungsoperatoren erzeugen beziehungsweise vernichten entsprechend diese Teilchen.\footnote{Dies ist nat\"urlich nur im entferntesten Sinn ein Teilchen, wie wir es gewohnt sind.}
 \iffalse 
Alternativ f\"uhren wir eine zus\"atzlich zu $\phi=0$ die Eichung:
\begin{equation*}
	\text{div}(a)=0
\end{equation*}
ein. Dies ist erlaubt da:
\begin{align}
	...
\end{align}
TODO
...
\end{theorem}
\begin{proof}
	Mit Hilfe eines Produktansatzes l\"asst sich die Schr\"odingergleichung des freien elektromagnetischen Feldes in $|V|-1$ eindimensionale freie und $|f|-1$ eindimensionale Schr\"dingergleichungen des harmonischen Oszillators zerlegen......
\end{proof}
...
\\
Als n\"achstes betrachten wir die Konsequenzen einer Mehrphotonendarstellung auf die messbaren Eigenschaften des elektromagnetischen Feldes.
\paragraph{Forderungen}: 
\begin{itemize}
	\item Sei $\psi\in \mathfrak{H}^n$ ein Zustand mit $n\geq 0$ Photonen, dann ist das gemessene elektromagnetische Feld gegeben als die Summe der Felder der einzelnen Photonen.\\
	\item Die Gesamtenergie des elektromagnetischen Feldes ist ebenso die Summe der Energien der einzelnen Photonen.
\end{itemize}
Diese Forderungen f\"uhrt direkt zu folgender Definition des Hamiltonoperators auf dem Fockraum.
\begin{definition}
Sei $G$ ein planarer Graph, $F(\mathfrak{H})$ der Fockraum auf $G$ und $H_1$ der Hamiltonoperator aus Definition 6.4, dann ist der Hamiltonoperator auf dem Fockraum gegeben durch:
\end{definition} 
\fi
\chapter{Die Klein-Gordongleichung}
Es verbleibt die Beschreibung des Stromes und der Ladung, eben der Quellen des elektromagnetischen Feldes. In der klassischen Theorie folgt f\"ur die beiden Gr\"o\ss{}en eine Kontinuit\"atsgleichung direkt aus den Maxwellgleichungen. Es verbleibt Gleichungen anzugeben, welche die Dynamik des Stromes und der Ladung selbst in Abh\"angingkeit des elektromagnetischen Feldes beschreiben. Der einfachste Weg dies zu bewerkstelligen ist mit Hilfe des Ohm'schen Gesetzes $j=\sigma e$.\\
\\
...
\begin{definition}
	Sei $G$ ein planarer Graph, $m>0$ eine reele Zahl, $r>0$ eine Kantenfunktion und $\Phi_0$ sowie $v_0$ komplexe Eckfunktionen, dann nennt man $\Phi\in C^1(\mathfrak{E}, \mathbb{C})$ eine L\"osung der \textbf{Klein-Gordongleichung} falls f\"ur alle $t\in\mathbb{R}$ gilt
	\begin{equation}
		\Phi^{\prime \prime} - \Delta_{r^{-2}}\Phi+ m \Phi = 0
	\end{equation}
...
\end{definition}
\noindent
Die Klein-Gordongleichung ist im kontinuierlichen Fall die einfachste Form einer Lorentz invariante Feldgleichung. Sie beschreibt ein skalares daher strukturloses Feld mit \\
\\
...
\begin{proposition}
	Sei $G$ ein planarer Graph, $m>0$ eine reele Zahl, $r>0$ eine Kantenfunktion und $\Phi$ eine L\"osung der Klein-Gordongleichung zur Anfangsbedingung $\phi_0\in\mathfrak{E}$ , dann gilt die Kontinuit\"atsgleichung:
	\begin{equation}
		q^\prime + \text{div}(j) = 0 
	\end{equation}
	...
	\begin{align}
		...
		&j = -\text{div}^{-1}\Im(\overline{\Phi}\Delta_{r^{-2}}(\Phi))
	\end{align}
\end{proposition}
\begin{proof}
Starten wir bei der linken Seite von (7.4)
\begin{align*}
	...
	\frac{1}{2i}(\overline{\Phi}*(\Delta_{r^{-2}}\Phi - m \Phi)-\Phi*\overline{(\Delta_{r^{-2}}\Phi - m \Phi)})=\\
	&\frac{1}{2i}(\overline{\Phi}*\Delta_{r^{-2}}\Phi-\Phi*\Delta_{r^{-2}}\overline{\Phi})=\Im(\overline{\Phi}*\Delta_{r^{-2}}(\Phi))
\end{align*}
Der letzte Term muss sich im Bild der Divergenz befinden um die Inverse der Divergenz darauf anzuwenden. Dies ist nach dem de'Rham Komplex Theorem genau dann der Fall wenn die Summe der Eckfunktion \"uber alle Ecken gleich Null ist. Sei hierzu  $\Phi(v)=a(v)+ib(v)$ f\"ur alle $v\in V$ und beachte, dass $\Delta_{r^{-2}}$ komponentenweise angewendet wird, dann gilt f\"ur den Imagin\"arteil
\begin{align*}
	&\sum_{v\in V} \Im(\overline{\Phi}*\Delta_{r^{-2}}\Phi)(v)=\\
	&\sum_{v\in V} (a(v)*\Delta_{r^{-2}} b(v) - b(v)*\Delta_{r^{-2}} a(v))=
	\langle a,\Delta_{r^{-2}} b\rangle_{\mathfrak{E}}-\langle b ,\Delta_{r^{-2}} a\rangle_{\mathfrak{E}}=\\
	&-\langle r^{-1}\text{grad}(a),r^{-1}\text{grad}(b)\rangle_{\mathfrak{K}}+\langle r^{-1}\text{grad}(b) , r^{-1}\text{grad}(a)\rangle_{\mathfrak{K}}=0
\end{align*}
Hiermit wurde gezeigt, dass sich der Imagin\"arteil im Bild der Divergenz befindet und somit mit $\text{div}\text{div}^{-1}$ erweitert werden kann, was uns zum gesuchten Resultat f\"uhrt.
\begin{equation*}
	q^\prime =\Im(\overline{\Phi}*\Delta_{r^{-2}}(\Phi))=\text{div}(\text{div}^{-1}(\Im(\overline{\Phi}*\Delta_{r^{-2}}(\Phi))))
\end{equation*}
\end{proof}
\noindent
Die Ladung $q$ und der Strom $j$ sind die Me\ss{}gr\"o\ss{}en der Klein-Gordon Theorie, w\"ahrend $\Phi$ analog zu den Potentialen der Maxwelltheorie ist.
\begin{proposition}
	Sei $G$ ein planarer Graph $m>0$ eine reele Zahl, $r>0$ eine Kantenfunktion, $\Phi_0$ sowie $v_0$ komplexe Eckfunktionen und $\pi_R, \pi_I, \Phi_R, \Phi_I\in C^1(\mathfrak{E}, \mathbb{R})$ und $\Phi=\Phi_R + i\Phi_I$ sowie $\pi=\pi_R + i\pi_I$, dann nennt man
	\begin{align*}
	H(\Phi_R, \Phi_I, &\pi_R, \pi_I) =\\ &\frac{1}{2}(\langle \pi_R, \pi_R\rangle_{\mathfrak{E}}+\langle r^{-1}\text{grad}(\Phi_R), r^{-1}\text{grad}(\Phi_R)\rangle_{\mathfrak{K}}+m^2\langle \Phi_R,\Phi_R\rangle_{\mathfrak{E}})\\
	&+ \frac{1}{2}(\langle \pi_I, \pi_I\rangle_{\mathfrak{E}}+\langle r^{-1}\text{grad}(\Phi_I), r^{-1}\text{grad}(\Phi_I)\rangle_{\mathfrak{K}}+m^2\langle \Phi_I,\Phi_I\rangle_{\mathfrak{E}})
	\end{align*}	
	die Hamiltonfunktion der Klein-Gordontheorie und $\Phi$ ist genau dann eine L\"osung der Klein-Gordongleichung sowie $\pi=\Phi^\prime$ wenn die Hamiltonschen Bewegungsgleichungen
	\begin{align}
		\Phi_R^\prime = \frac{\partial H}{\partial \pi_R}\\
		\pi_R^\prime = - \frac{\partial H}{\partial \Phi_R}\\
		\Phi_I^\prime = \frac{\partial H}{\partial \pi_I}\\
		\pi_I^\prime = - \frac{\partial H}{\partial \Phi_I}
	\end{align} erf\"ullt sind.
\end{proposition}
\begin{proof}
TODO
\end{proof}
TODO Existenz Lösungen
TODO Teilchen Antiteilchen Koordinatentransformation inklusive ladungskonjugation
TODO Energieerhaltung
\\
Wir kommen nun zur Quantisierung der Klein-Gordontheorie. Wie bereits bei den Maxwellgleichungen definieren wir einen Hilbertraum.
\begin{definition}
	Sei $G$ ein planarer Graph, dann ist der Hilbertraum unserer Theorie gegeben durch
	\begin{equation}
		\mathfrak{H}(G) = L^2(\mathfrak{E}(G)^2,\mathbb{C})
	\end{equation}
	Der Raum der komplexen Funktionen auf $\mathfrak{E}(G)$ deren Quadrate ein endliches Integral aufweisen.\\
	\\
	Das Skalarprodukt auf $\mathfrak{H}$ f\"ur $\phi,\xi\in \mathfrak{H}$ ist hier definiert durch:
	\begin{equation}
		\langle \phi, \xi\rangle_{\mathfrak{H}} = \int_{\mathfrak{E}^2} \overline{\phi}(q)\xi(x)d\lambda(x)
	\end{equation}
\end{definition}
\noindent
...
\begin{definition}
	Sei $G$ ein planarer Graph, $m>0$ eine reele Zahl und $r>0$ eine Kantenfunktion, dann ist der der Hamiltonoperator des Klein-Gordon Feldes gegeben durch:
	\begin{equation}
		\hat{H}=-\sum_{k\in V}\frac{1}{2}\frac{\partial^2}{\partial \Phi_k^2} + \frac{1}{2}(\langle r^{-1}\text{grad}(\Phi), r^{-1}\text{grad}(\Phi)\rangle_{\mathfrak{K}}+ m^2\langle\Phi, \Phi\rangle_{\mathfrak{E}})
	\end{equation}
	Sei $\psi_0\in\mathfrak{H}$ und $\psi\in C^1(\mathbb{R},\mathfrak{H})$, sodass:
	\begin{equation}
		\hat{H}\psi(t) = i\psi^\prime(t)
...
\noindent
\begin{theorem}
	Sei $G$ ein planarer Graph, $r>0$ eine Kantenfunktion, dann gibt es eine eindeutige L\"osung der Schr\"odingergleichung des Klein-Gordonfeldes.
\end{theorem}
\noindent
...
\\ 
Um die Klein-Gordontheorie mit der Maxwelltheorie zu verbinden fordern wir die so genannte lokale $U(1)$-Eichinvarianz. Dies bedeutet, dass wir f\"ur die Lagrangefunktion $L$ der Theorie Invarianz unter folgender Transformation fordern:
\begin{equation}
\gamma_\xi(\left(
\begin{array}{c}\phi\\
	a\\
...
\end{equation} 
\begin{definition}
	Sei $G$ ein planarer Graph, $A$ eine Kantenfunktion und $\Phi$ eine Eckfunktion, dann ist der Eichgradient definiert durch:
	\begin{equation}
		\text{grad}^A(\Phi)(e) = \Phi(p(e)) - \Phi(q(e))e^{iA(e)}
	\end{equation}
\end{definition}

\begin{align*}
	L(\phi, a, \Phi, \phi^\prime, a^\prime, \Phi^\prime) =&\frac{1}{2}(\langle \overline{\Phi^\prime + i \phi}, \Phi^\prime + i \phi\rangle_{\mathfrak{E}}-
	\langle r^{-1}\overline{\text{grad}^A(\Phi)}, r^{-1}\text{grad}^A(\Phi)\rangle_{\mathfrak{K}}\\
	& - m^2\langle \overline{\Phi}, \Phi\rangle_{\mathfrak{E}}+ \langle \epsilon(a^\prime + r\text{grad}(\phi)), a^\prime + r\text{grad}(\phi)\rangle_\mathfrak{K} -\\& \langle\text{div}_D(r^{-1}a), \text{div}_D(r^{-1}a)/\mu\rangle_\mathfrak{F})
\end{align*}
\chapter*{Abschluss}
