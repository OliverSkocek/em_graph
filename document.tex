...
\begin{definition}
	Sei $G$ ein planarer Graph, $r>0$, $a$ Kantenfunktionen und $\Phi$ eine komplexe Eckfunktion, dann ist der \textbf{Eichgradient} f\"ur jede Kante $\tilde{e}\in E$ definiert durch:
	\begin{equation}
		\text{grad}_r^a(\Phi)(\tilde{e}) = \Phi(p(\tilde{e})) - \Phi(q(\tilde{e}))e^{i\frac{a}{r}(\tilde{e})}
	\end{equation}
	Sei weiters $r>0$ eine Kantenfunktion und $v\in V$ , eine Ecke dann nennen wir 
		\begin{equation}
		\Delta^a_r(\Phi)(v) = (\text{grad}_r^{a\dagger}*r^{-2}*\text{grad}_r^a)(\Phi)(v)
	\end{equation} 
	den Eich-Laplace Operator. Man beachte das sich die Adjunktion $\dagger$ auf das komplexe Skalarprodukt bezieht.
\end{definition}
\begin{lemma}
	Sei $G$ ein planarer Graph, $r>0$, $a$ reele Kantenfunktionen und $\xi$ eine reele Eckfunktion.  F\"ur jede komplexe Eckfunktion $\Phi$ gilt dann f\"ur jede Kante $\tilde{e}$.
	\begin{equation}
		\text{grad}_r^{a+r*\text{grad}(\xi)}(e^{i\xi}\Phi)(\tilde{e})=e^{i\xi(p(\tilde{e}))}\text{grad}_r^a(\Phi)(\tilde{e})
	\end{equation}
\end{lemma}
\begin{proof}
	Sei $\tilde{e}\in E$
	\begin{align*}
		\text{grad}_r^{a+r*\text{grad}(\xi)}(e^{i\xi}\Phi)(\tilde{e}) &= e^{i\xi(p(\tilde{e}))}\Phi(p(\tilde{e})) - e^{i\xi(q(\tilde{e}))}(q(\tilde{e}))e^{i(\frac{a}{r}+\text{grad}(\xi))(\tilde{e})}\\
		&= e^{i\xi(p(\tilde{e}))}(\Phi(p(\tilde{e})) - \Phi(q(\tilde{e}))e^{i\frac{a}{r}(\tilde{e})})\\
...
L\"osung der Klein-Gordongleichung
	\begin{equation}
	(\frac{\partial}{\partial t} - i(\phi-\xi^\prime))(\frac{\partial}{\partial t} + i(\phi-\xi^\prime))\tilde{\Phi} - \Delta^{a + r\text{grad}\xi}_r\tilde{\Phi}+ m^2 \tilde{\Phi} = 0
\end{equation}
\end{proposition}
\begin{proof}
...
\begin{proposition}
	...
	\begin{equation}
	H(\Phi, \pi) = \frac{1}{2}(\langle \pi, \pi\rangle_{\mathfrak{E},\mathbb{C}}+\langle r^{-1}\text{grad}_r^a(\Phi), r^{-1}\text{grad}_r^a(\Phi)\rangle_{\mathfrak{K},\mathbb{C}}+m^2\langle \Phi,\Phi\rangle_{\mathfrak{E},\mathbb{C}})
	\end{equation}	
...
\begin{definition}
	...
	\begin{equation}
		\hat{H}=-\sum_{k\in V}2\overline{\partial_k}\partial_k + \frac{1}{2}(\langle r^{-1}\text{grad}_r^a(\Phi), r^{-1}\text{grad}_r^a(\Phi)\rangle_{\mathfrak{K}}+ m^2\langle\Phi, \Phi\rangle_{\mathfrak{E}})
	\end{equation}
...
\noindent
\begin{theorem}
	Sei $G$ ein planarer Graph, $r>0$, $a$ Kantenfunktionen, dann gibt es eine eindeutige L\"osung der Schr\"odingergleichung des Klein-Gordonfeldes.
\end{theorem}
\begin{proof}
	content...
\end{proof}
