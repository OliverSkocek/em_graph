...
\\
Bevor wir jedoch endlich $\mathfrak{H}_{qed}$ definieren, folgt noch eine Bemerkung dar\"uber was passieren w\"urde wenn wir den verbleibenden Eichgruppen Freiheitsgrad von $\psi$ fixieren, nämlich jener  bez\"uglich $\xi\in \text{kern}(r\text{grad})$ beziehungsweise konstantem $\xi$.
\begin{definition}
...
\end{definition}
\begin{lemma}
	Sei $G$ ein planarer Graph und $y\in E$ beliebig, dann gilt $\psi\in \mathfrak{H}_{\text{sym}}$ genau dann wenn es ein $f\in L^2(\mathbb{R}^{\{y\}}\times\mathbb{C}^{V-\{y\}}, \mathbb{C})$ gibt sodass f\"ur alle $\Phi\in\mathfrak{E}_\mathbb{C}(G)$ gilt
	\begin{equation}
...
\end{proof}
\noindent
Dieser Satz gibt uns eine explizite Konstruktion des Raums der Funktionen $\phi\in L^2(\mathfrak{E}_\mathbb{C}(G), \mathbb{C})$, die invariant unter globaler Rotation sind, daher unter Rotation jeder Ecke um denselben Winkel. Diese Funktionen lassen sich auch \"uber ein anderes Kriterium identifizieren.
\begin{remark}
...
\end{proof}
\noindent
Daher handelt es sich bei $\mathfrak{H}_{\text{sym}}$ um den Teilraum der Wellenfunktionen, die Zust\"ande mit Gesamtladung 0 darstellen. Wir wollen diese Einschr\"ankung nicht vornehmen.\\
\\
Als n\"achstes k\"onnen wir endlich den Hilbertraum der skalaren Quantenelektrodynamik definieren.
\begin{definition}
	Sei $G$ ein planarer Graph, $r>0\in\mathfrak{K}$ und $\psi\in L^2(\mathfrak{E}_{\mathbb{C}}\times(\mathfrak{T}+\text{kern}(\text{div r})),\mathbb{C})$ sowie $u$ wie in Satz 10.2, dann sagen wir $\psi\in \mathfrak{H}_{qed}^{pre}$ genau dann wenn es ein $\tilde{\psi}\in C^1(\mathfrak{E}_\mathbb{C}(G)\times\mathbb{C}^{|A|-1}, \mathbb{C})$ gibt sodass  $\psi$ sich \"uber $\tilde{\psi}$ nach (10.10) darstellen l\"asst als
	\begin{equation}
