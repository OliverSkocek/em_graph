...
\begin{proposition}
	...
	\end{equation}
	Wobei f\"ur $Q$ gelten soll, dass $Qr\text{grad}=P_{\text{Bild}(r\text{div})}$ und $r\text{grad}Q=P_{\text{Bild}(r\text{grad})}$ ist\footnote{Das entspricht der Pseudoinversen.}.
\end{proposition}
\begin{proof}
\end{proof}
\noindent
...
\end{proposition}
\begin{proof} Betrachte die Menge
	\begin{equation}
		\mathfrak{T} = r\text{grad}(\{\phi\in\mathfrak{E}(G):\sum_{x\in V}\phi_x=0, -\pi\leq\phi<\pi\})
	\end{equation}
und schreibe:
\begin{align*}
\text{Bild}(r\text{grad}) &= r\text{grad}(\{\phi\in\mathfrak{E}(G):\sum_{x\in V}\phi_x=0, -\pi\leq\phi<\pi\} + 2\pi\mathfrak{E}(G)_\mathbb{Z})\\
&=\mathfrak{T} + 2\pi*r\text{grad}(\mathfrak{E}(G)_\mathbb{Z})
\end{align*}
\end{proof}
\noindent
