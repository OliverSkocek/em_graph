...
\begin{definition}
	...
	\begin{equation}
		i\,\frac{\mathrm{d}}{\mathrm{d}t}\psi(t) = \hat{H}(t)\psi(t), \label{eq:schroedinger}
	\end{equation}
...
\begin{theorem}[Ehrenfest-Theorem]
	...
	\(\psi: \mathbb{R} \to \mathfrak{H}(G)\) eine Lösung der Schrödingergleichung \eqref{eq:schroedinger}.  
	\\
	\\
...
TODO\\
\fi
Bislang fehlt noch die Bestätigung, dass die Schrödingergleichung \eqref{eq:schroedinger} 
...
\begin{proposition}[Kontinuitätsgleichung]\label{prop:kontinuitaet}
	...
	\begin{align}
		q(t) &= \Im\!\bigl(\overline{\Phi(t)}\,\Phi'(t)\bigr), \label{eq:kg_qdef}\\[4pt]
...
\begin{proof}
	...
	\begin{align*}
		q'(t)
		&= \bigl(\Im(\overline{\Phi}\,\Phi')\bigr)' 
		= \!\left(
		\frac{\overline{\Phi}\,\Phi' - \Phi\,\overline{\Phi}'}{2i}
		\right)' \\[4pt]
		&= \frac{1}{2i}
		\bigl(
		...
		= \frac{1}{2i}
		\bigl(
		\overline{\Phi}\,\Phi'' - \Phi\,\overline{\Phi''}
		\bigr).
	\end{align*}
	\\
	...
	\begin{align*}
		q'
		&= \frac{1}{2i}
		\bigl(\Phi\,\overline{\Delta_r^{\,a}(\Phi)}
		- \overline{\Phi}\,\Delta_r^{\,a}(\Phi)\bigr)
...
\begin{proof}
	...
	\[
	\tilde{q} 
	= \Im(\overline{\tilde{\Phi}}\,\tilde{\Phi}')
	= \Im\!\left(\overline{e^{i\xi}\Phi}\, e^{i\xi}\Phi'\right)
	= \Im(\overline{\Phi}\,\Phi')
	= q.
	\]
	\\
...
\begin{proof}
	...
	\begin{equation*}
		q_{\overline{\Phi}}
		=  \Im(\overline{\overline{\Phi}}\,\overline{\Phi}')
		=  \Im\!\bigl(\overline{\overline{\Phi}\,\Phi'}\bigr)
		=  -\,\Im(\overline{\Phi}\,\Phi')
		= -\,q_{\Phi}.
	\end{equation*}
	\\
...
\begin{align}
	q &= \frac{\overline{\Phi}\,\pi - \Phi\,\overline{\pi}}{2i} \nonumber\\
	&= \frac{(\Phi_R - i\,\Phi_I)(\pi_R + i\,\pi_I) - (\Phi_R + i\,\Phi_I)(\pi_R - i\,\pi_I)}{2i} \nonumber\\
	&= \Phi_R\,\pi_I - \Phi_I\,\pi_R. \label{eq:KG-Ladung-reell}
\end{align}
\noindent
...
\begin{definition}[Ladungsfeldoperator und Gesamtladungsoperator]\label{def:KG-Ladungsoperator}
	...
	\begin{align}\label{eq:KG-Ladungsfeldoperator}
		\hat{q}(x) &= \hat{\Phi}_R(x)\,\hat{\pi}_I(x) - \hat{\Phi}_I(x)\,\hat{\pi}_R(x)\\
		&= i\left( \hat{\Phi}_I(x)\frac{\partial}{\partial \Phi_R(x)} - \hat{\Phi}_R(x)\frac{\partial}{\partial \Phi_I(x)} \right)\nonumber
	\end{align}
...
\begin{theorem}\label{thm:KG-QM-Kontinuitaet}
	Sei $G$ ein \textbf{planares Netz} und $\psi\in\mathfrak{D}_{\mathrm{KG}}\subset\mathfrak{H}_{\mathrm{KG}}(G)$. 
	Dann gilt für jeden Knoten $x\in V$ und den Ladungsfeldoperator $\hat{q}(x)$ 
	die Kontinuitätsgleichung:
	\begin{equation}\label{eq:KG-QM-Kontinuitaet}
		\langle \psi(t), \hat{q}(x) \psi(t) \rangle_{\mathfrak{H}}^\prime
		= -\,\mathrm{div}\Bigl( \langle \psi(t), \hat{j} \psi(t) \rangle_{\mathfrak{H}} \Bigr)(x),
	\end{equation}
	wobei der Stromoperator durch
...
\end{theorem}
\begin{proof}
	Wir starten auf der linken Seite von \eqref{eq:KG-QM-Kontinuitaet} und gehen vom Ehrenfest Theorem der Klein-Gordon Schrödingergleichung aus.
	\begin{equation}
		\langle \psi, \hat{q}(x)\psi\rangle_\mathfrak{H}^\prime=i\langle \psi, [\hat{H},\hat{q}(x)]\psi\rangle_\mathfrak{H}
	\end{equation}
	Wie im Fall der Maxwell Theorie sei hier kurz eine Skizze des Beweises.\footnote{Der volle Beweis ist etwas technischer und kann in "On the Ehrenfest Theorem of quantum mechanics, Gero Friesecke und Mario Koppen, 2009" gefunden werden.}\\
	Sei $x$ ein Knoten des Graphen und $\psi$ eine L\"osung der Schr\"odingergleichung~\ref{eq:KG-Schroedinger}.
	\begin{align*}
		...
		&= \langle -i\hat{H}\psi, \hat{q}(x)\psi\rangle_\mathfrak{H} + \langle \psi, -i\hat{q}(x)\hat{H}\psi\rangle_\mathfrak{H}\\
		&= i(\langle \psi, \hat{H}\hat{q}(x)\psi\rangle_\mathfrak{H} - \langle \psi, \hat{q}(x)\hat{H}\psi\rangle_\mathfrak{H})\\
		&=i\langle \psi, [\hat{H},\hat{q}(x)]\psi\rangle_\mathfrak{H} 
	\end{align*}
	Als n\"achstes betrachten wir den Kommutator $[\hat{H},\hat{q}(x)]$:
	\begin{align*}
	...
	\end{align*}
	Wegen der Form von $\hat{q}$ verschwindet der Kommutator mit den Ausdr\"ucken $\hat{\pi}_{\Phi_R}(y)^2 + \hat{\pi}_{\Phi_I}(y)^2$, $\hat{\Phi}_R(y)^2 + \hat{\Phi}_I(y)^2$. Dies l\"asst sich an einem Beispiel illustrieren:
	\begin{align*}
...
\end{proof}
\noindent
Im nächsten Schritt wollen wir, wie in der Maxwell Theorie, Erzeugungs und Vernichtungsoperatoren als Leiteroperatoren eines mehrdimensionalen harmonische Oszillators definieren. Wir starten mit den Modenoperatoren\eqref{eq:KG-hj-Wirtinger}.
	\begin{equation}
	\hat{h}_j =  
	-2\frac{\partial^2}{\partial \Phi_j \partial \overline{\Phi}_j} 
	+ \frac{1}{2}(m^2 + \omega_j^2) \Phi_j\overline{\Phi_j}	
\end{equation} 
\noindent
Es handelt sich hierbei um einen eindimensionalen komplexen harmonischen Oszillator beziehungsweise um einen zweidimensionalen reelen harmonischen Oszillator.
 \begin{equation}
 	\hat{h}_j= \frac{1}{2}(-\frac{\partial^2}{\partial \Phi_{Rj}^2}-\frac{\partial^2}{\partial \Phi_{Ij}^2} + (m^2 + \omega_j^2) (\Phi_{Rj}^2 + \Phi_{Ij}^2)
 \end{equation}
Wir können analog zu Definition~\ref{def:Vernichtung}je einen Vernichtungsoperator f\"ur den Realteil $\Phi_R$ und den Imagin\"arteil $\Phi_I$ der Summanden $h_\omega$  definieren, wobei $\Phi = \Phi_R +i\Phi_I$.
\begin{align*}
	\hat{r}_j &= \sqrt{\frac{\sqrt{m^2 + \omega_j^2}}{2}}(\Phi_{Rj} -\frac{1}{\sqrt{m^2 + \omega_j^2}}\frac{\partial}{\partial\Phi_{Rj}})\\
	\hat{s}_j &= \sqrt{\frac{\sqrt{m^2 + \omega_j^2}}{2}}(\Phi_{Ij} -\frac{1}{\sqrt{m^2 + \omega_j^2}}\frac{\partial}{\partial\Phi_{Ij}})
\end{align*}
\noindent
Der deutliche Unterschied zum elektromagnetischen Feld ist, dass wir hier für jede Mode $\omega$ zwei unterschiedliche Vernichter und Erzeuger haben. Nach Hilfssatz~\ref{thm:Teilcheanzahl} lässt sich $\hat{h}_j$ schreiben als
\begin{equation*}
	\hat{h}_j = \sqrt{m^2 + \omega_j^2}(\hat{r}_j^\dagger\hat{r}_j + \hat{s}_j^\dagger\hat{s}_j + 1)\label{eq:rf-h}
\end{equation*}
Das quantisierte Klein-Gordon Feld l\"asst sich also durch zwei Arten von Teilchen darstellen. Eine die von $\hat{r}$ vernichtet wird und eine die durch $\hat{s}$ vernichtet wird. Dies sind fast die bekannten Teilchen und Antiteilchen. Es ergeben sich folgende Vertauschungen.
\begin{align}
	[\hat{r}_i, \hat{s}_j^\dagger] &= 0\label{eq:rf-comm}\\
	[\hat{r}_i, \hat{s}_j] &= 0\nonumber\\
	[\hat{r}_i, \hat{r}_j^\dagger] &= \delta_{i,j}1\nonumber\\
	[\hat{s}_i, \hat{s}_j^\dagger] &= \delta_{i,j}1\nonumber
\end{align}
Es gibt jedoch mehrere Möglichkeiten diese Erzeuger und Vernichter Paare zu definieren, sodass sie sowohl \ref{eq:rf-comm} als auch \ref{eq:rf-h} erfüllen.
\begin{remark}
	Sei \(a,b,c,d\in\mathbb{C}\), dann erfüllen \(\hat{u} = a\hat{r} + b\hat{s}\) und \(\hat{v} = c\hat{r} + c\hat{s}\) genau dann \ref{eq:rf-comm} und \ref{eq:rf-h}
	falls
	\[
	\begin{pmatrix}
		a & b \\
...
\end{remark}
\noindent
Wir werden als Vernichter und Erzeuger eine Darstellung wählen, die es auch erlaubt den Ladungsoperator über die Teilchenanzahl zu interpretieren.
\begin{align}
	\hat{A}_j &= \frac{1}{\sqrt{2}}(\hat{r}_j + i\hat{s}_j) = \frac{\sqrt[4]{m^2 + \omega_j^2}}{2}\Phi_{j} - \frac{1}{\sqrt[4]{m^2 + \omega_j^2}}\frac{\partial}{\partial\overline{\Phi}_{j}}\\
	\hat{B}_j &= \frac{1}{\sqrt{2}}(\hat{r}_j - i\hat{s}_j) = \frac{\sqrt[4]{m^2 + \omega_j^2}}{2}\overline{\Phi}_{j} - \frac{1}{\sqrt[4]{m^2 + \omega_j^2}}\frac{\partial}{\partial\Phi_{j}}
\end{align}
Die Adjungierten $\hat{A}_j^\dagger$, $\hat{B}_j^\dagger$ sind die Erzeugungsoperatoren. Analog zum eindimensionalen Fall definieren wir die \textbf{Teilchenzahloperatoren des Klein-Gordon-Feldes}.
\begin{align*}
	\hat{N}_{j +} &= \hat{A}_j^\dagger \hat{A}_j\\
	\hat{N}_{_j -} &= \hat{B}_j^\dagger \hat{B}_j
\end{align*}
Der Operator $\hat{A}_j^\dagger$ erh\"oht die Teilchenzahl $\hat{N}_{j +}$ um Eins, während der Operator $\hat{A}_j$ dieselbe um Eins verringert. Dasselbe gilt f\"ur $\hat{B}_j$, $\hat{N}_{j -}$ und $\hat{B}_j^\dagger$.\\
\\
Wir nennen $\hat{N}_{j +}$ den Teilchenzahloperator und $\hat{N}_{j -}$ den Antiteilchenzahloperator.\\
\\
Der Grundzustand $\psi_{0j}=c\textrm{ }e^{-\sqrt{m^2 + \omega_j^2} |\Phi_j|^2 / 2}$ wird sowohl von $\hat{A}_j$ als auch von $\hat{B}_j$ vernichtet. 
\begin{equation*}
	\hat{A}_j\psi_{0j} = \hat{B}_j\psi_{0j} = 0
\end{equation*}
Dar\"uberhinaus kann gezeigt werden, dass Teilchen die durch $\hat{A}_j$ vernichtet werden eine positive Ladung tragen und Teilchen die druch $\hat{B}_j$ vernichtet werden eine negative Ladung tragen.
\begin{proposition}
	Sei $G$ ein planares Netz und $\hat{N}_{\omega\pm}$ wie oben, dann k\"onnen wir den Gesamtladungsoperator folgenderma\ss{}en schreiben
	\begin{equation*}
		\hat{Q} = \sum_{j=1}^{|V|}(\hat{N}_{j +} -\hat{N}_{j -
		})
	\end{equation*}
...
Starten wir mit Definition~\ref{def:KG-Ladungsoperator} des Gesamtladungsoperators und formen um:
	\begin{align*}
		\hat{Q} = \sum_{x\in V} \hat{q}(x)&=i\sum_{x\in V} \left( \hat{\Phi}_I(x)\frac{\partial}{\partial \Phi_R(x)} - \hat{\Phi}_R(x)\frac{\partial}{\partial \Phi_I(x)} \right)\\
		&=\sum_{x\in V} \left(\frac{\partial}{\partial\Phi_{x}}\Phi_x-\overline{\Phi}_x\frac{\partial}{\partial\overline{\Phi_{x}}} -1\right)\\
		&=\sum_{x\in V} \left(\Phi_x\frac{\partial}{\partial\Phi_{x}}-\overline{\Phi}_x\frac{\partial}{\partial\overline{\Phi_{x}}} \right)
\end{align*}
...
\begin{align*}
	\hat{Q}\hat{U}\psi(\Phi) &= \sum_{x\in V}\left( \Phi_x\frac{\partial}{\partial\Phi_{x}}\psi(U(\Phi))-\overline{\Phi}_x\frac{\partial}{\partial\overline{\Phi_{x}}}\psi(U(\Phi))\right)\\
	&= \sum_{x\in V} \left(\sum_{j=1}^{|V|}\Phi_x U_{j,x}(\frac{\partial}{\partial\Phi_{j}}\psi)(U(\Phi))-\sum_{j=1}^{|V|}\overline{\Phi}_x\overline{U_{j,x}}(\frac{\partial}{\partial\overline{\Phi_{j}}}\psi)(U(\Phi))\right)\\
\end{align*}	
Dann wenden wir \(\hat{U}^\dagger\) an:
\begin{align*}
	\hat{U}^\dagger\hat{Q}\hat{U}\psi(\Phi) &= \sum_{x\in V} \left(\sum_{j=1}^{|V|}\sum_{k=1}^{|V|}\overline{U_{k,x}} U_{j,x}\Phi_k\frac{\partial}{\partial\Phi_{j}}\psi(\Phi)-\sum_{j=1}^{|V|}\sum_{k=1}^{|V|}U_{k,x}\overline{U_{j,x}}\overline{\Phi}_k\frac{\partial}{\partial\overline{\Phi_{j}}}\psi(\Phi)\right)\\
	&= \sum_{x\in V} \left(\sum_{j=1}^{|V|}\sum_{k=1}^{|V|}\overline{U_{k,x}} \overline{U^\dagger_{x,j}}\Phi_k\frac{\partial}{\partial\Phi_{j}}\psi(\Phi)-\sum_{j=1}^{|V|}\sum_{k=1}^{|V|}U_{k,x}U^\dagger_{x,j}\overline{\Phi}_k\frac{\partial}{\partial\overline{\Phi_{j}}}\psi(\Phi)\right)\\
	&=\sum_{k=1}^{|V|}\left(\Phi_k\frac{\partial}{\partial\Phi_{k}}\psi(\Phi)-\overline{\Phi}_k\frac{\partial}{\partial\overline{\Phi_{k}}}\psi(\Phi)\right)
\end{align*}
...
\begin{align*}
	...
	=\sum_{j=1}^{|V|}&((\frac{w_j}{4}\Phi_j\overline{\Phi}_j +\frac{1}{2}\Phi_j\frac{\partial}{\partial\Phi_j}-\frac{1}{2}\frac{\partial}{\partial\overline{\Phi}_j}\overline{\Phi}_j-\frac{1}{w_j}\frac{\partial^2}{\partial\Phi_j\partial\overline{\Phi}_j})\\&-(\frac{w_j}{4}\overline{\Phi}_j\Phi_j +\frac{1}{2}\overline{\Phi}_j\frac{\partial}{\partial\overline{\Phi}_j}-\frac{1}{2}\frac{\partial}{\partial\Phi_j}\Phi_j-\frac{1}{w_j}\frac{\partial^2}{\partial\overline{\Phi}_j\partial\Phi_j}))\\
	=\sum_{j=1}^{|V|}&(\Phi_j\frac{\partial}{\partial\Phi_j}-\overline{\Phi}_j\frac{\partial}{\partial\overline{\Phi}_j})
\end{align*}
...
\begin{proposition}
	...
	\begin{align}
	U_CA_j U_C &= B_j\\
	U_CA^\dagger_j U_C &= B^\dagger_j\\
	U_CB_j U_C &= A_j\\
	U_CB^\dagger_j U_C &= A^\dagger_j\\
	U_C N_{j \pm} U_C &= N_{j \mp}
\end{align}
\end{proposition}
\begin{proof}
	Es genügt zu zeigen, dass \(U_CA_j U_C=B_j\)\\
	\\
	Sei \(\psi \in \mathfrak{D}_{\mathrm{KG}}\), dann gilt:
	\begin{align}
		U_CA_j U_C\psi(\Phi)&=U_C(\frac{\sqrt[4]{m^2 + \omega_j^2}}{2}\Phi_{j} - \frac{1}{\sqrt[4]{m^2 + \omega_j^2}}\frac{\partial}{\partial\overline{\Phi}_{j}})\psi(\overline{\Phi})\\
		&=U_C(\frac{\sqrt[4]{m^2 + \omega_j^2}}{2}\Phi_{j} - \frac{1}{\sqrt[4]{m^2 + \omega_j^2}}\frac{\partial}{\partial{\Phi}_{j}}\psi)(\overline{\Phi})
		\\
		&=(\frac{\sqrt[4]{m^2 + \omega_j^2}}{2}\overline{\Phi}_{j} - \frac{1}{\sqrt[4]{m^2 + \omega_j^2}}\frac{\partial}{\partial{\Phi}_{j}}\psi)(\Phi)=B_j\psi(\Phi)
	\end{align}
\end{proof}
