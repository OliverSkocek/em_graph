...
\end{pmatrix}
\end{equation}
Das Matrixexponential kann als Potenzreihe oder mit Hilfe der Jordanschen Normalform der Matrix $M=\begin{pmatrix}
	0 & \mathbb{I} \\ 
	L & 0
\end{pmatrix}$ konstruiert werden.
\\
\\
...
v \\ 0
\end{pmatrix}$ von $M$ konstruieren.
...
\begin{lemma}
	Sei $L$ eine positiv semidefinite lineare Abbildung in $V=\mathbb{R}^n$ ($n\geq 1 $), $m>0$ und $u\in C^2(\mathbb{R}, V)$ eine L\"osung der Gleichung
\begin{equation}
...
\end{equation}
und umgekehrt ist die erste Komponente einer L\"osung von (7.26) auch eine L\"osung von (7.24).
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{proposition}
	Sei $G$, $m>0$, $r>0$ wie in Definition 7.3 und $a$, $a_0$ und $e_0$ Kantenfunktionen, dann gibt es eine eindeutige L\"osung des Anfangswertproblems.
	\begin{align}
		\frac{\partial^2}{\partial t^2}\Phi - \Delta^{a}_r\Phi&+ m^2 \Phi = 0\\
		a(0)&=a_0\\
		a^\prime(0)&=e_0
...
\end{proposition}
\begin{proof}
	Der lineare Operator $L = - \Delta^{a}_r+ m^2$ ist nach seiner Konstruktion ein positiv definiter linearer Operator.\\
	\\
	Daher k\"onnen wir Hilfsatz 7.7 anwenden und finden eine L\"osung 
	\begin{equation*}
		\psi(t) = e^{i Ht}\psi_0
	\end{equation*} 
	des \"aquivalenten Anfangswertproblems 
	\begin{align*}
	i\frac{\partial\psi}{\partial t} &= H\psi\\
	\psi(0)&=\psi_0
	\end{align*}
	...
	\begin{equation*}
		\psi_0 =\begin{pmatrix}
			a_0 \\ (L^\frac{1}{2}a_0 + i e_0)/m
			  \end{pmatrix}
	\end{equation*}
\end{proof}
\noindent
Der Operator $H$ ist indefinit und seine Eigenwerte sind symmetrisch um Null. Da er selbstadjungiert ist bleibt die Norm von $\psi$ unter der Klein-Gordon Zeitentwicklung erhalten.\\
Dies bedeutet, dass wir eine Wahrscheinlichkeitsdichte $\rho=\overline{\psi}\psi$ definieren k\"onnen, die nichtnegativ ist und deren Summe $\sum_{v\in V}\rho(v)$ \"uber die Zeit erhalten bleibt, daher es gibt eine zugeh\"orige Kontinuit\"atsgleichung. \\
\\
Die Voraussetzung $m>0$ ist hier tats\"achlich notwendig, da im Fall $m=0$ und nichttrivialen Kern von $L$ die Konstruktion einer solchen Wahrscheinlichkeit und damit Assoziation mit einem Knoten(Ort) nicht m\"oglich ist.
\\
\\
Es gibt keine Ein-Photon Schr\"odingergleichung!
