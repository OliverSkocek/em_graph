...
\begin{definition}
	Sei $G$ ein planarer Graph, $r>0,\epsilon\in\mathfrak{K}$, $\mathfrak{H}_{qed}$ ein passender Hilbertraum, dann nennen wir den Operator $\hat{H}_{qed}:D(\hat{H}_{qed})\subseteq\mathfrak{H}_{qed}\mapsto\mathfrak{H}_{qed}$
	\begin{align}
		\begin{split}
...
\begin{proposition}
	Sei $G$ ein planarer Graph, $A$ die Fl\"achen von $G$ und $\psi\in F(\mathfrak{E}_\mathbb{C}(G)\times\mathfrak{K}(G), \mathbb{C})$, dann ist $\psi$ genau dann invariant unter $\hat{U}_\xi$ f\"ur beliebiges $\xi\in\mathfrak{E}(G)$ mit $\sum_{x\in V}\xi(x)=0$ falls es ein $f\in F(\mathfrak{E}_\mathbb{C}(G)\times\mathbb{C}^{|A|-1}, \mathbb{C})$ gibt sodass f\"ur alle $\Phi\in\mathfrak{E}_\mathbb{C}(G)$ und $a\in\mathfrak{K}(G)$ gilt
	\begin{equation}
		\psi(\Phi, a) = f(\Phi e^{-iQ(a)}, u(a))
	\end{equation}
	Wobei $Q$ die Pseudoinverse von $r\text{grad}$ und $u:\mathfrak{K}(G)\rightarrow \mathbb{C}^{|A|-1}$ ist, sodass $uu^T=1_{\mathbb{C}^{|A|-1}}$ und $u^Tu=P_{\text{kern}(\text{div r})}$.
\end{proposition}
\begin{proof}
Sei $\psi\in F(\mathfrak{E}_\mathbb{C}(G)\times\mathfrak{K}(G), \mathbb{C})$ f\"ur beliebiges $\xi\in\mathfrak{E}(G)$ mit $\sum_{x\in V}\xi(x)=0$ invariant, dann definieren wir $f\in F(\mathfrak{E}_\mathbb{C}(G)\times\mathbb{C}^{|A|-1}, \mathbb{C})$ durch 
\begin{equation*}
	f(.,.) = \psi(.,u^T(.))
\end{equation*}
...
\\
Bevor wir jedoch endlich $\mathfrak{H}_{qed}$ definieren, f\"uhren wir noch einen weiteren Begriff ein und geben einen damit assoziierten Darstelllungsatz mit dem wir den verbleibenden Eichgruppen Freiheitsgrad von $\psi$ fixieren, nämlich jener  bez\"uglich $\xi\in \text{kern}(r\text{grad})^\perp$.
\begin{definition}
	Sei $G$ ein planarer Graph, dann definieren wir
	\begin{equation}
		\mathfrak{H}_{\text{sym}}=\{\psi\in F(\mathfrak{E}_\mathbb{C}(G), \mathbb{C})| \hat{U}_\xi\psi=\psi \forall \xi\in \text{kern}(r\text{grad})^\perp\}
	\end{equation}

\end{definition}
\begin{lemma}
	Sei $G$ ein planarer Graph, dann gilt $\psi\in \mathfrak{H}_{\text{sym}}$ genau dann wenn es ein $f\in F(\mathbb{R}^{\{y\}}\times\mathbb{C}^{V-\{y\}}, \mathbb{C})$ gibt sodass f\"ur alle $\Phi\in\mathfrak{E}_\mathbb{C}(G)$ gilt
	\begin{equation}
		\psi(\Phi) = f(\Phi e^{-i\arg(\Phi)(y)})
	\end{equation}
\end{lemma}
\begin{proof}
	Analog zum Beweis von Satz 10.2.
\end{proof}
\noindent
Dieser Satz gibt uns eine explizite Konstruktion des Raums der Funktionen $\phi\in F(\mathfrak{E}_\mathbb{C}(G), \mathbb{C})$, die invariant unter globaler Rotation sind, daher unter Rotation jeder Ecke um denselben Winkel. Diese Funktionen lassen sich auch \"uber ein anderes Kriterium identifizieren.
\begin{lemma}
	Sei $G$ ein planarer Graph, dann gilt $\psi\in \mathfrak{H}_{\text{sym}}$ genau dann wenn
	\begin{equation}
		\sum_{x\in V}\frac{\partial}{\partial \phi(x)}\psi = 0
	\end{equation}
\end{lemma}
\begin{proof}
	Betrachten wir zun\"achst die Abbildung $\hat{U}_\xi$ mit $\xi\in\text{kern}(r\text{grad})^\perp$, daher $\xi$ ist eine Konstante. Analog zu Satz 10.1 berechnen wir den infinitesimalen Erzeuger zu $\hat{U}_\xi$. Sei hierzu $\psi\in \mathfrak{H}_{\text{sym}}$.
	\begin{align*}
...
\end{definition}
\begin{remark}
	Die Einschr\"ankung auf $\mathfrak{E}_{\mathbb{C}}\times(\mathfrak{T}+\text{kern}(\text{div r}))$ der Funktion $\psi$ ist durch periodische Fortsetzung umkehrbar nach Folgerung 10.4. Entsprechend k\"onnen wir alle bisher behandelten Operatoren \"uber diese Fortsetzung wie gehabt auf $\mathfrak{H}_{\text{qed}}$ definieren.
\end{remark}
\noindent
\begin{proposition}
	Sei $\psi\in\mathfrak{H}_{\text{qed}}$ wie in Definition 10.6, dann gilt 
...
\end{align*}
Nach Hilfsatz 10.7 gilt nach Vorraussetzung auch
	\begin{equation*}
	\sum_{x\in V}\frac{\partial}{\partial \phi(x)}\psi = 0
	\end{equation*}
