...
\begin{corollary}
	...
	\end{equation}
	dann ist die L\"osung der Schr\"dingergleichung $\psi$ entlang jeder Richtung des gerade definierten Raums $A$ konstant.
	\begin{equation}
...
\\
Der elektrische Feldoperator $\hat{e}$ verh\"alt sich wegen (6.12, 6.23) unter \"Ahnlichkeitstransformation durch $\hat{U}$ folgenderma\ss{}en:
\begin{equation*}
	\hat{U}^\dagger \hat{e}\hat{U} = U^T * \hat{e}
\end{equation*}
Wenn wir nun $\text{div}(r*\epsilon * .)$ von links anwenden ($\epsilon =1$), dann sind die ersten $|V| -1 $ Komponenten des transformierten Feldoperators Null.
\end{proof}
\iffalse
