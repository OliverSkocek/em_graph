...
\\
Zeigen wir nun $\Omega\subset L^2(\mathbb{R}, \mathbb{C},\lambda)$.\\
Falls $f$ in $\Omega$ liegt ist auch das Betragsquadrat $|f|^2$ in $\Omega$, somit reicht es zu zeigen, dass die Funktionen in $\Omega$ integrierbar sind.\\
Die Funktionen sind genau dann integrierbar, wenn f\"ur alle $n\in\mathbb{N}$ gilt, dass
\begin{equation*}
	x\mapsto x^ne^{-x^2/2}
\end{equation*}
integrierbar ist. Dies kann durch partielle Integration und einen Grenz\"ubergang gezeigt werden.\\
\\
Um zu zeigen, dass der Raum $\Omega$ dicht in $L^2(\mathbb{R}, \mathbb{C},\lambda)$ liegt, gen\"ugt es sich davon zu \"uberzeugen, dass beliebige stetige Funktionen mit kompaktem Tr\"ager mit beliebig approximier bar sind mit Elementen aus $\Omega$.\\
Sei $g\in C_c(\mathbb{R}, \mathbb{C})$ eine komplexe stetige Funktion mit kompaktem Tr\"ager, und $\text{supp}(g)$ der kompakte Tr\"ager. Sei $u\in \Omega$, dann k\"onnen wir ein kompaktes Intervall $[-R,R]$ w\"ahlen sodass 


Der Beweis f\"ur den laut 6.32 adjungierten Operator geht analog.
\end{proof}
\noindent
Den adjungierten Operator $\hat{a}_\omega^\dagger$ nennen wir den \textbf{Erzeugungsoperator}.\\
...
\begin{proposition}
	Das Spektrum des Teilchenzahloperator $\hat{N}_\omega$ ist die Menge der nat\"urlichen Zahlen
	\begin{equation}
		\sigma(\hat{N}_\omega)=\mathbb{N}
	\end{equation}
