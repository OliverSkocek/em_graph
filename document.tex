...
\end{proof}
\begin{proposition}
Sei $G$ ein planarer Graph mit Orientierung, $r>0$, $\epsilon>0$, $a_0$, $e_0$ Kantenfunktionen, $\mu>0$ eine Fl\"achenfunktion auf $G$, dann gibt es eine Kurve $a\in C^2(\mathbb{R},\mathfrak{K})$, sodass f\"ur alle $t\in\mathbb{R}$ gilt
\begin{equation}
...
\begin{align*}
	...
	&\langle r^{-1}u/ \epsilon, -\text{grad}_D(\text{div}_D(r^{-1}u) / \mu) \rangle_{\mathfrak{K}}=\\
	&\langle \text{div}_D(r^{-1}u/ \epsilon), \text{div}_D(r^{-1}u) / \mu\rangle_{\mathfrak{F}}
\end{align*}
\end{proof}
...
\end{pmatrix}
\end{equation}
Untersuchen wir die Eigenschaften des Matrixexponentials $e^{	\begin{pmatrix}
		0 & \mathbb{I} \\ 
		Q & 0
	\end{pmatrix}t}$ indem wir das Spektrum der Matrix $M=\begin{pmatrix}
0 & \mathbb{I} \\ 
Q & 0
\end{pmatrix}$ betrachten.
\\
\\
Sei $\omega\in\sigma(Q)$ ein Eigenwert von $Q$ mit Eigenvektor $v_\omega$. Falls $\omega\neq 0$, dann k\"onnen wir zwei Eigenvektoren $u_{\pm\omega}=\begin{pmatrix}
	v_\omega \\ \pm\sqrt{\omega}v_\omega
\end{pmatrix}$ von $M$ konstruieren. Im Fall $\omega=0$ k\"onnen wir f\"ur jeden zugeh\"origen Eigenvektor $v$ nur einen einzigen Eigenvektoren $u=\begin{pmatrix}
v \\ 0
