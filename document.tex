...
\end{definition}
\noindent
\paragraph{Interpretation}Jedes Element $\psi\in\mathfrak{H}$ stellt einen m\"oglichen Zustand des quantisierten elektromagnetischen Feldes dar. Jeder Messgr\"o\ss{}e $\Omega$ des klassischen elektromagnetischen Feldes entspricht ein Operator $\hat{\Omega}$ auf $\mathfrak{H}$. Der Ausgang einer Messung der Gr\"o\ss{}e $\Omega$ ist zuf\"allig und der Mittelwert der Messungen ist gleich $\langle \psi, \hat{\Omega}\psi\rangle_{\mathfrak{H}}$.
\\
\\
Zwischen den Operator Kurven $\hat{\pi}$ und $\hat{q}$ wird dann nach Heisenberg die kanonische Vertauschungsrelation gefordert:
\begin{equation}
	[\hat{q}(e), \hat{\pi}(e')] = i\delta_{e, e'}
\end{equation}
f\"ur jede Kante $e\in E$.\\
\\
Eine m\"ogliche Darstellung dieser Operatoren $\hat{q}(k)$ und $\hat{\pi}(k)$ ist f\"ur jedes $k\in E$ gegeben durch:
\begin{align}
	\hat{\pi}(k):(q\mapsto\psi(q)) \mapsto (q\mapsto i\frac{\partial\psi}{\partial q_k}(q))\\
	\hat{q}(k):(q\mapsto\psi(q)) \mapsto (q\mapsto q_k\psi(q))
\end{align}
\noindent
...
\\
\\
Eine kurze Betrachtung der Hamiltonfunktion (6.4) zeigt eine \"Aquivalenz des Problems zu einer $|E|$ Punktma\ss{}en mit Massen $r(k)$ f\"ur jedes $k\in E$ in einem Potential. Der Hamiltonoperator des elektromagnetischen Feldes auf dem planaren Graphen $G$ l\"asst sich daher schreiben als:
\begin{equation}
...
\begin{theorem}[Ehrenfest Theorem]
	Sei $G$ ein planarer Graph, $\epsilon>0$ und $r>0$ Kantenfunktionen, $\mu>0$ eine Fl\"achenfunktion und $\psi: \mathbb{R}\mapsto \mathfrak{H}$ eine L\"osung der Schr\"odingergleichung auf $G$, dann gilt:
	\begin{align}
