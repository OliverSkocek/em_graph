...
\noindent
\begin{definition}
	Sei $G$ ein planarer Graph mit Orientierung, dann nennt man eine Folge von Ecken $(x_0, x_1,\dots,x_n)$, sodass auf einander folgende Ecken immer durch eine Kante verbunden sind, einen \textbf{Pfad} auf $G$. Falls $x_0 = x_n$ gilt, nennt man dies einenen \textbf{geschlossenen Pfad}.
\end{definition}
...
\end{proof}
\begin{definition}
	Sei $G$ ein planarer Graph mit Orientierung und $f\in\mathfrak{K}(G)$, $(x_i)_{i=1}^n$ ein Pfad auf $G$, $(e_i)_{i=1}^{n-1}$ die Folge der Kanten die f\"ur jedes $1\leq i \leq n-1$ $x_i$ und $x_{i+1}$ verbindet und es sei $\sigma_i$ f\"ur jedes $1\leq i \leq n-1$ gleich $-1$ falls $p(e_i)=x_i$ ist und sonst $+1$, dann nennt man die Summe
...
\end{proposition}
\begin{proof}
Zu allererst reicht es \textbf{einfach geschlossene Pfad}e zu betrachten daher jene in denen nur das erste Glied mehrfach und zwar doppelt auftaucht. Diese Einschränkung ist zul\"assig weil sich f\"ur alle anderen geschlossenen Pfade die Pfadsumme sich als Summe solcher einfach geschlossenen Pfade schreiben l\"asst.\\
\\
...
\begin{proof}[Beweis de Rham Komplex]
	...
	\\
	\textbf{Erste zu beweisende Aussage:} Der Kern von $\text{grad}$ sind genau die konstanten Eckfunktionen.\\
	\\
	Dies wurde bereits im Satz 3.1 gezeigt.\\
	\\
	\textbf{Zweite zu beweisende Aussage:} Das Bild des $\text{grad}$ ist genau der Kern von $\text{div}_D$.
	\\
	...
	$$div_D(\text{grad}(f)) = \sum_{i=1}^{k-1} \sigma(i) * \sigma(i) * (f(x_{(i+1) \text{ mod } k})-f(x_{i  \text{ mod } k}))$$
	...
	Es fehlt noch zu zeigen, dass es zu jedem Element im Kern von $\text{div}_D$ auch im Bild von $\text{grad}$ ist.\\
	...
	W\"ahlen wir nun eine beliebige Ecke $x_0$ und weil $G$ zusammenh\"angend ist, k\"onnen wir von $x_0$ zu jeder Ecke in $G$ einen Pfad bilden. Definiere nun $F\in\mathfrak(E)(G)$, sodass f\"ur jede Ecke $x$ $F$ als Wert die Pfadsumme eines Pfades von $x_0$ nach $x$ hat. Es gilt nun $\text{grad}(F) = f$.
	\\
	\\
	...
	Wegen dem Satz 2.1 und der ersten Aussage in diesem Beweis folgt die Aussage.
\end{proof}
\noindent
...
\begin{proof}
	...
	Sei $A$ die Untermatrix die man durch streichen der zu $\partial$ geh\"orenden Zeilen und Spalten erh\"alt und sei $x$ ein Element von $\text{kern}(A)$, daher $A(x)=0$. Wir k\"oennen ein Element $\hat{x}$ von $\mathfrak{E}$ konstruieren, sodass $\hat{x}$ an den Ecken die zu $\partial$ geh\"oren Null gesetzt wird und sonst den Wert annimmt den $x$ an der entsprechenden Ecke hat. Es gilt dann:
